  ...
  (
    ...
    React.useEffect(() => {
      const handleKeyDown = (event: KeyboardEvent) => {
        ...
        if (isModifier && event.key === SIDEBAR_KEYBOARD_SHORTCUT) {
      ...
      >
        ...
        <div
          className={cn(
            ...
            "group-data-[collapsible=offcanvas]:data-[side=left]:left-[calc(var(--sidebar-width)*-1)] group-data-[collapsible=offcanvas]:data-[side=right]:right-[calc(var(--sidebar-width)*-1)]",
            "group-data-[collapsible=icon]:w-[--sidebar-width-icon]",
            "group-data-[variant=floating]:p-2 group-data-[variant=floating]:group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+theme(spacing.4))]",
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">تغییر وضعیت سایدبار</span>
    </Button>
  );
});
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "group-data-[collapsible=offcanvas]:group-data-[side=left]:-right-2 group-data-[collapsible=offcanvas]:group-data-[side=right]:left-0",
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute end-3 top-3 flex aspect-square h-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2",
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute end-1 top-1.5 flex aspect-square h-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2",
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      "absolute end-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium text-sidebar-foreground select-none pointer-events-none",
      "peer-data-[size=sm]/menu-button:top-1 peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
...
>(({ className, showIcon = false, ...props }, ref) => {
  const width = React.useMemo(() => `${Math.floor(Math.random() * 41) + 50}%`, []);
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground",
